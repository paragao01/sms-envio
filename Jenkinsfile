            ...
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: 'dev', keyFileVariable: 'key')]) {
                 script {
                    ...
                    sshCommand remote: remote, command: " echo nexus63987 | sudo -S docker container run --network intranet -v /opt/envio-sms:/opt/envio-sms -h sms-envio -d --name sms-envio -p 8083:8083 177.53.20.16:5001/sms-envio:${BUILD_NUMBER}"
                 }
              }
            }
        }  
